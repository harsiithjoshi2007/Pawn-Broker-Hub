...
export default function NewCustomer() {
  ...
  const onSubmit = async (data: CustomerFormValues) => {
    try {
      ...
      const result = await createCustomer.mutateAsync({ data: payload as any });
      toast({ title: "Customer Created", description: `${result.name} has been added to the system.` });
      queryClient.invalidateQueries({ queryKey: getListCustomersQueryKey() });
      setLocation(`/customers/${result.id}`);
    } catch (error: any) {
      toast({ variant: "destructive", title: "Failed to create customer", description: error.message || "An unexpected error occurred." });
    }
  };
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-10">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Create a new customer profile to issue loans.</p>
        </div>
      </div>
